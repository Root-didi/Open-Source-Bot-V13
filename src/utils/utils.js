...
async function guildload(client, con, message) {
    await con.query(`INSERT INTO guilds (active, guildid, prefix) VALUES ('true', '${message.guild.id}', '${client.config.prefix}')`, async (err, row) => {
        if(err) throw err;
    });
};
...
async function permCheck(client, message, role) {
    ...
    switch(role) {
        case "managers":
            ...
            break;
        case "moderation":
            per = client.config.permcheck.moderation
            break;
        case "support":
